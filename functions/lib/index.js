...
exports.sendLandlordInvite = exports.enforceFreemiumGate = exports.generateExport = exports.createMonthlyBills = exports.sendRentReminders = exports.onUserDeleted = void 0;
...
exports.sendRentReminders = functions.pubsub
    ...
    const messages = [];
    await Promise.allSettled(leasesSnap.docs.map(async (leaseDoc) => {
        const lease = leaseDoc.data();
        const { userId, address, dueDay, monthlyRent, leaseEnd } = lease;
        if (leaseEnd && new Date(leaseEnd) < today)
            return;
        ...
        if (!pushToken || !reminderDays.includes(daysUntilDue))
            return;
        messages.push({
            to: pushToken,
            title: 'Rent Due Soon',
...
// ── createMonthlyBills ─────────────────────────────────────────────────────────
exports.createMonthlyBills = functions.pubsub
    .schedule('0 8 15 * *')
    .timeZone('America/New_York')
    .onRun(async () => {
    ...
    await Promise.allSettled(householdsSnap.docs.map(async (householdDoc) => {
        ...
        const existing = await billsRef
            .where('dueDate', '>=', admin.firestore.Timestamp.fromDate(new Date(nextMonthYear, nextMonthIndex, 1)))
            .where('dueDate', '<', admin.firestore.Timestamp.fromDate(new Date(nextMonthYear, nextMonthIndex + 1, 1)))
            .limit(1)
            .get();
        if (!existing.empty) {
            functions.logger.info(`[createMonthlyBills] Bill already exists for household=${hid} month=${monthLabel}`);
            return;
        }
        const newBill = {
        ...
        functions.logger.info(`[createMonthlyBills] Created bill for household=${hid} label="${newBill.label}"`);
    }));
...
exports.generateExport = functions.https.onCall(async (data, context) => {
    ...
    const rows = (await Promise.all(paymentsSnap.docs.map(async (payDoc) => {
        const p = payDoc.data();
        const confirmedAt = toDate(p.confirmedAt);
        if (confirmedAt < fromDate || confirmedAt > endDate)
            return null;
        let receiptUrl = null;
        if (p.receiptStoragePath) {
            try {
                ...
                functions.logger.warn(`[generateExport] Could not sign receipt for payment ${payDoc.id}`);
            }
        }
    ...
    await new Promise((resolve, reject) => {
        ...
        doc.text(`Period: ${fromDate.toLocaleDateString('en-US')} – ${endDate.toLocaleDateString('en-US')}`, { align: 'center' });
        doc.moveDown(1);
        // Column widths
        ...
        for (const row of rows) {
            ...
            doc.text(`$${row.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, { width: COL.amount, continued: true });
        ...
        doc.moveTo(startX, y).lineTo(startX + COL.date + COL.amount + COL.method + COL.receipt, y).stroke();
        y += 6;
        doc.font('Helvetica-Bold').fontSize(10).text(`Total Paid: $${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, startX, y, { align: 'right' });
        doc.end();
    });
...
exports.enforceFreemiumGate = functions.firestore
    ...
    if (leasesSnap.size > 1) {
        functions.logger.warn(`[enforceFreemiumGate] uid=${userId} exceeded lease limit — deleting payment ${paymentId}`);
    ...
    if (months.size > 12) {
        functions.logger.warn(`[enforceFreemiumGate] uid=${userId} exceeded 12-month history — deleting payment ${paymentId}`);
        await snap.ref.delete();
        await db.collection('users').doc(userId).collection('freemiumRejections').add({
...
exports.sendLandlordInvite = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    const userId = context.auth.uid;
